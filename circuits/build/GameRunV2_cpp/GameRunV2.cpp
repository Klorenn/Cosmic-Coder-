...
void GameRunV2_3_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void GameRunV2_3_run(uint ctx_index,Circom_CalcWit* ctx);
...
ctx->componentMemory[coffset].templateName = "GameRunV2";
ctx->componentMemory[coffset].signalStart = soffset;
...
GreaterEqThan_2_create(mySignalStart+25,0+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[0] = 0+ctx_index+1;
}
{
PFrElement aux_dest = &signalValues[mySignalStart + 24];
// load src
Fr_mul(&expaux[0],&signalValues[mySignalStart + 15],&circuitConstants[5]); // line circom 25
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
GameRunV2_3_create(1,0,ctx,"main",0);
GameRunV2_3_run(0,ctx);
}
